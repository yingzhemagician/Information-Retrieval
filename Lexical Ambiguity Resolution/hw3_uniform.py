...
for word in open(doc_path, 'r'):
    ...
    elif word in common_words or not re.search("[a-zA-Z]", word) or word[:3] == ".x-":# if the path change, this "word ==" should be changed
        pass
    else:
        if word in new_doc_vec.keys():
            weight = new_doc_vec.get(word)
            new_doc_vec.setdefault(word, weight + 1)
        else:
            new_doc_vec.setdefault(word, 1)
